...
```{r}
gini_nc_ts <- eu_data_nested |>
  unnest(NUTS3_data) |>
  mutate(Year = as.integer(time)) |>
  group_by(nc, nc_name, Year) |>
  summarise(
    gini_nc = DescTools::Gini(gdp_pc_n3, weights = pop_n3, na.rm = TRUE),
    .groups = "drop"
  )
...
```{r}
land_grupper <- gini_nc_ts |>
  group_by(nc, nc_name) |>
  summarise(
    first_year = min(Year, na.rm = TRUE),
    gini_first = gini_nc[Year == first_year][1],
    gini_2022  = gini_nc[Year == 2022][1],
    .groups = "drop"
  ) |>
  mutate(
    gruppe = case_when(
      is.na(gini_first) | is.na(gini_2022) ~ NA_character_,
      gini_2022 < gini_first ~ "Lavere",
      gini_2022 > gini_first ~ "Høyere",
      TRUE ~ "Lik"
    )
  )
```
...
```{r}
gini_nc_ts2 <- gini_nc_ts |>
  left_join(land_grupper |> select(nc, gruppe), by = "nc") |>
  filter(!is.na(gruppe), gruppe != "Lik")
```

```{r}
gini_nc_ts2 |>
  filter(gruppe == "Lavere") |>
  ggplot(aes(x = Year, y = gini_nc, group = nc, colour = nc_name)) +
  geom_line() +
  labs(x = "Year", y = "gini_nc", colour = "nc_name")
```

```{r}
gini_nc_ts2 |>
  filter(gruppe == "Høyere") |>
  ggplot(aes(x = Year, y = gini_nc, group = nc, colour = nc_name)) +
  geom_line() +
  labs(x = "Year", y = "gini_nc", colour = "nc_name")
```
...
eu_data_nested |>
  ...
  ggplot(aes(x = Year, y = gini_n2, group = n2, colour = n2)) +
  geom_line() +
  labs(x = "Year", y = "gini_n2", colour = "n2")
```
...
eu_data_nested |>
  ...
  ggplot(aes(x = Year, y = gini_n1, group = n1, colour = n1)) +
  geom_line() +
  labs(x = "Year", y = "gini_n1", colour = "n1") +
  theme(legend.position = "bottom")
```
...
```{r}
fr_nuts1_gini_2022 <- eu_data_nested |>
  unnest(NUTS3_data) |>
  mutate(Year = as.integer(time)) |>
  filter(nc == "FR") |>
  group_by(n1, Year) |>
...
fr_nuts1_gini_2022 |>
  mutate(gini_n1 = round(gini_n1, 3)) |>
  as_flextable()
```
...
```{r}
fr1_nuts3_ts <- eu_data_nested |>
  unnest(NUTS3_data) |>
  mutate(Year = as.integer(time)) |>
  filter(nc == "FR", n1 == "FR1") |>
  select(n3, Year, gdp_pc_n3)
...
```{r}
fr1_nuts3_2022 <- fr1_nuts3_ts |>
  filter(Year == 2022) |>
  arrange(desc(gdp_pc_n3)) |>
  mutate(gdp_pc_n3 = round(gdp_pc_n3, 0))
...
```{r}
#Lag mod_sum og mod_coeff som list-columns (en per NUTS2-region)
...
Svaret visar et flertall av NUTS2-regionene på ca 61 %, som er sammenhengen mellom endringer i BNP per innbygger og endringer i regional ulikhet positiv.
...
p_mod <- plm(
  diff_gini_nuts2 ~ diff_gdp_per_capita,
  data  = unnest(select(NUTS2_diff,
 n2, NUTS2_diff), NUTS2_diff), 
 index = c("n3", "time"),
  model = "pooling"
)
...
Resultatet fra panel-regresjonen viser at det er en positiv sammenheng mellom endringer i BNP per innbygger og endringer i regional ulikhet (Gini-koeffisienten).
Det betyr at når BNP per innbygger øker, så har også den regionale ulikheten en tendens til å øke noe.

Sammenhengen er statistisk signifikant, altså lite sannsynlig for at den bare skyldes tilfeldigheter.
Samtidig er effekten liten, og BNP-vekst alene forklarer bare en liten del av forskjellene i regional ulikhet.

Resultatet må derfor tolkes forsiktig.
Selv om økonomisk vekst ofte henger sammen med økte regionale forskjeller, finnes det også mange andre faktorer som påvirker/bidrar hvordan verdiskapningen fordeles mellom regioner.
...
Her ble det gjort enpanelanalyse med faste effekter, der vi kun ser på hvordan variablene utvikler seg innen hver region over tid.
Dette betyr at alle konstante forskjeller mellom regionene fjernes, slik at analysen fokuserer på endringer fra år til år.
Det som skjer er at det blir brukt robuste standardfeil for å sikre at resultatene er pålitelige selv om variansen i dataene ikke er helt jevn.
Resultatet viser her at når BNP per innbygger øker, vil også den regionale ulikheten ha en vist tendesn til å øke litt.
Sammenhengen er statistisk signifikant.
Sammenlignet med en ordinær regresjon uten faste effekter gir denne modellen samme hovedkonklusjon, men her er det en strengere metode som kun bruker utviklingen innen regionene over tid.
